 ...
 * @author user
 */
public class AdonanRotiTawar extends Roti implements HargaFillTop{
    ...
    public double hAdonanPcs(){
        double hPcs = this.totalHargaAdonan() * 400 / this.totalBeratAdonan();
        return hPcs;
    }
    
    public void tampilRincianAdonan(){
       System.out.println("Total berat adonan Roti Tawar    = " + String.format("% .0f",this.totalBeratAdonan()) + " gr");
       System.out.println("Total harga adonan Roti Tawar    = " + "Rp " + String.format("% .0f",this.totalHargaAdonan()));
       System.out.println("Total harga adonan per pcs       = "  + "Rp " + String.format("% .0f",this.hAdonanPcs()));
       System.out.println("---------------------------------------------------"); 
    }
    
    public void tampilRincianFillTop(){
       System.out.println("Total harga fill+top Roti Tawar  = " + "Rp " + String.format("% .0f", this.totalHargaFillTop()));
       System.out.println("Total harga bahan per pcs        = " + "Rp " + String.format("% .0f",(this.hAdonanPcs() + this.totalHargaFillTop())));
       System.out.println("Harga jual Roti Tawar per pcs    = " + "Rp " + String.format("% .0f", ((this.hAdonanPcs() + this.totalHargaFillTop()) * 1.5)));
       System.out.println(" \n" + " ");
    }
   ...
   @Override
    public double totalBeratTerigu() {
        double mTerigu = this.terigu/this.totalBeratAdonan()*400*35;
        return mTerigu;
    }

    @Override
    public double totalBeratGula() {
      double mGula = this.gula/this.totalBeratAdonan()*400*35;
        return mGula;  
    }

    @Override
    public double totalBeratButter() {
       double mButter = this.butter/this.totalBeratAdonan()*400*35;
        return mButter; 
    }

    @Override
    public double totalBeratRagi() {
        double mRagi = this.ragi/this.totalBeratAdonan()*400*35;
        return mRagi;
    }

    @Override
    public double totalBeratSubu() {
        double mSubu = this.suBu/this.totalBeratAdonan()*400*35;
        return mSubu;
    }

    @Override
    public double totalBeratSuca() {
        double mSuca = this.suCa/this.totalBeratAdonan()*400*35;
        return mSuca;
    }

    @Override
    public double totalBeratTelur() {
        double mTelur = this.telur/this.totalBeratAdonan()*400*35;
        return mTelur;
    }

    @Override
    public double totalBeratEs() {
        double mEs = this.es/this.totalBeratAdonan()*400*35;
        return mEs;
    }
